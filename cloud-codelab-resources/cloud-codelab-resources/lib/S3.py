...
# modified by davew to use urlfetch.fetch
...
class AWSAuthConnection:
    ...
    def _make_request(self, method, bucket='', key='', query_args={}, headers={}, data='', metadata={}):
        ...
        # convert method str to urlfetch method int id
        methodId = urlfetch.GET
        if method == 'POST':
            methodId = urlfetch.POST
        elif method == 'HEAD':
            methodId = urlfetch.HEAD
        elif method == 'PUT':
            methodId = urlfetch.PUT
        elif method == 'DELETE':
            methodId = urlfetch.DELETE
        
        # fetch(url, payload=None, method=GET, headers={}, allow_truncated=False)
        return urlfetch.fetch(url, data, methodId, final_headers, False)
    ...
    def _add_aws_auth_header(self, headers, method, bucket, key, query_args):
        ...
        if not headers.has_key('x-amz-date'):
            headers['x-amz-date'] = time.strftime("%a, %d %b %Y %X GMT", time.gmtime())
...
class Response:
    def __init__(self, http_response):
        ...
        if http_response.status_code >= 300 and self.body:
            ...
            self.message = "%03d %s" % (http_response.status_code, http_response.content)
